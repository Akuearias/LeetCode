...
LeetCode Exercise #2140. Solving Questions With Brainpower
...
class Solution:
    def mostPoints(self, questions: List[List[int]]) -> int:
        total_scores = [0] * (len(questions)+1)

        for i in range(len(questions)-1, -1, -1):
            total_scores[i] = max(total_scores[i+1], total_scores[min(len(questions), i + questions[i][1] + 1)] + questions[i][0])
